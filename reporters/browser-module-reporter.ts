...
/** Playwright's config.rootDir is testDir; workspace root is the config file directory. */
...
export default class BrowserModuleReporter implements Reporter {
  private projectRoot = '';
  private rows: Row[] = [];
  ...
  onEnd(result: FullResult) {
    ...
    const runId =
      process.env.LAPUS_RUN_VERSION ||
      manifest.runId ||
      `run-${Date.now()}`;
    const dirName = safeSegment(runId);
    const outDir = path.join(this.projectRoot, 'reports', dirName);
    ...
    const byBrowser: Record<
      string,
      Record<
        string,
        { passed: number; failed: number; skipped: number; timedOut: number; interrupted: number }
      >
    > = {};

    for (const r of this.rows) {
      byBrowser[r.browser] ??= {};
      byBrowser[r.browser][r.module] ??= {
        passed: 0,
        failed: 0,
        skipped: 0,
        timedOut: 0,
      ...
      const bucket = byBrowser[r.browser][r.module];
      ...
      else if (r.status === 'interrupted') bucket.interrupted += 1;
    }

    const browsersInRun = [...new Set(this.rows.map((r) => r.browser))].sort();
    ...
    const latestDir = path.join(this.projectRoot, 'reports', 'latest');
...
<title>Lapus test report — ${esc(runId)}</title>
<style>
...
.meta{color:#8b949e;font-size:0.9rem;margin-bottom:16px}
    ...
    html += `Started: ${esc(String(summary.startedAt || ''))}<br/>`;
